...
license: MIT
platform: universal
description: "Diagnose and triage cron job failures. Checks job states, identifies error patterns, prioritizes by criticality, and generates health reports. Use when cron jobs are failing, tasks aren't running, or you need a cron health check. Triggers on: 'cron failures', 'job health check', 'scheduled task errors', 'job not running'. NOT for: creating new cron jobs (use the cron tool directly)."
---
